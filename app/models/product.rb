class Product < ActiveRecord::Base
  ...
  def self.get_all
    ...
    self.where("products.status=1")
      ...
      .where("products.cache_last_ordered IS NOT NULL AND products.cache_last_ordered >= ?", Time.now - 6.months)
      .where("products.cache_last_priced IS NOT NULL AND products.cache_last_priced >= ?", Time.now - 6.months)
  end
